...
const getSearchBarcodeItemCodeDetails: any = async (request: any, karigar_name: any, get_access_token: any) => {
    ...
    await axios
        .get(
            `${CONSTANTS.API_BASE_URL}/api/method/sj_antique.sdk.api?version=v1&method=get_item_wise_barcode_filter&entity=barcode&posting_date=${request.date}&custom_karigar=${karigar_name}&name=${request.item_group}&sr_from=${request.sr_no_from}&sr_to=${request.sr_no_to}
            `,
            getHeaders
        )
        .then((res: any) => {
