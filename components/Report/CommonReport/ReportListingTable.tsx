...
const ReportListingTable = ({ headers, reportData, tableViewData }: any) => {
  ...
  return (
    ...
    {reportData.length > 0 && (
      <tr
        className="row row-cols-7 justify-content-center text-center fw-bold "
      >
        <td scope="col" className="col-1 table_row py-1 py-auto bg-primary bg-opacity-25">
        ...
        {headers.map((header: any, idx: any) => (
          <td
            key={idx}
            scope="col"
            className={`table_row py-1 py-auto bg-primary bg-opacity-25 ${
              headers?.length <= 4 ? 'col-2' : 'col'
            }`}
          >
